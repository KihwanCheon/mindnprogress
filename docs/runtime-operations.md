...
- 상위 `MindNProgress_Start.vbs`와 `MindNProgress_Start.bat`는 **재시작 후 브라우저 열기**, `MindNProgress_Stop.bat`는 **종료만** 수행한다. 모두 `scripts/mnp-runtime.ps1`을 호출한다.
- 사용자가 진행 과정을 보고 실행하려면 상위 `MindNProgress_Start.bat`를 연다. VBS와 동일한 재시작·호환 옵션·완료 대기·브라우저 열기를 사용하되, 공통 제어 스크립트의 진행 로그와 오류를 콘솔에 표시한다. 완료 또는 실패 후 키 입력을 기다려 결과를 확인할 수 있으며 원래 종료 코드를 보존한다. 기존 VBS와 상시 실행 서버의 숨김 동작은 바꾸지 않는다.
- 시작 호출 설정은 `MindNProgress_Start.bat` 한 곳에서 관리한다. BAT는 인수가 없으면 콘솔 모드, `--hidden` 옵션이면 숨김·키 입력 대기 생략 모드로 같은 제어 명령을 실행한다. VBS는 BAT를 `--hidden` 옵션으로 숨겨 실행하고 완료 대기·실패 대화상자 표시만 담당한다. 두 파일의 주석에 수정 책임과 원본 동기화 규칙을 명시했으며, 옵션·종료 코드 계약 변경 시에는 양쪽을 함께 확인한다.
...
사전·최종 포트 소유권 검사는 콘솔을 만들지 않는 `netstat -ano` 조회로 통일한다. TCP 수신 항목만 추려 IPv4·IPv6와 PID를 확인한다. `-p tcp`는 IPv6를 누락하므로 사용하지 않는다. NetTCPIP 공급자가 실제 수신 포트를 빈 결과로 반환해 시작 전 검사가 실패하는 경우에 의존하지 않으며, 조회 실패·빈 출력·해석할 수 없는 TCP 행은 포트가 비었다고 간주하지 않는다. 정상 조회에서 대상 수신 포트가 없는 경우만 빈 목록으로 처리한다. 기존 NHN 소유권·명령줄·생성 시각·부모 관계 검증은 그대로 유지한다.

HTTP 준비 검사는 전체 마감 안에서 **개별 시도를 최대 5초**로 제한하고, 멈춘 요청 이후에도 새 연결로 다시 확인한다. 이는 최종 검증 전체를 5초로 제한하거나 시작 제한을 늘리는 것이 아니다. 900ms보다 느린 정상 응답도 허용하며, 서버나 예약 작업을 다시 시작하지 않는다. 대기 중 `[http-wait]`에 웹/API 상태와 조회 시간을 표시하고, 한쪽만 실패해도 다른 쪽의 완료된 상태 코드를 로그에 보존한다.
...
`runtime-entrypoints.test.mjs`는 콘솔 배치와 VBS → 공통 배치 경로를 임시 설치 경로의 가짜 제어 스크립트로 실행한다. 공백·특수 문자가 있는 경로, 실행 디렉터리, 공통 제어 인수, 표준 출력/오류 표시, 숨김 실행, 성공·실패 종료 코드 및 제어 파일 누락 안내를 확인하며 실제 서버 재시작이나 브라우저 열기는 수행하지 않는다. WSH 비대화형 모드(`//B`)에서는 `MsgBox`가 종료 코드를 0으로 바꾸므로, 해당 모드에서만 대화상자를 생략해 실패 코드를 보존한다. 사용자 직접 실행의 오류 대화상자는 유지한다.

`runtime-controller.test.mjs`의 포트 회귀 검사는 실제 `Get-MnpSnapshot`을 호출해 공급자 오류·빈 포트·다른 PID/계정·조회 오류를 검사하고, Windows PowerShell 5.1에서 실제 임시 IPv4·IPv6 수신 포트와 소유 PID도 조회한다. `runtime-http-readiness.test.mjs`는 1.8초 정상 응답, 영구 무응답의 전체 제한, 한쪽만 실패한 진단 정보, 첫 요청만 멈춘 뒤 다음 요청에서 정상 복구되는 경우를 웹/API 각각 검증한다. 운영 서버 중단 없이 실행한다.

`runtime-task-host.test.mjs`는 프로세스 생성 **이전부터** 별도 메시지 처리 스레드의 Windows SHOW 이벤트 훅을 가동한다. 화면에 나타나지 않는 메시지 전용 창으로 감시기 자체를 확인한 뒤, GUI 호스트를 10회 실행하면서 창 표시 이벤트·부모 관계·작업 디렉터리·서버 종료까지의 대기·정상/실패 종료 코드 전달을 검사한다. 실제 시작 VBS → 공통 배치 → 격리 제어 PowerShell → Node/netstat 조회 → GUI 호스트 경로도 검사한다. 테스트 부모가 호스트 창을 대신 숨겨 거짓으로 통과하지 않도록 GUI 호스트 실행에는 숨김 플래그를 주지 않는다. 운영 예약 작업을 직접 실행한 검증과는 구분해 기록한다.
