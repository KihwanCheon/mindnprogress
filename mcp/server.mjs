...
const productGuide = {
  ...
  operationRules: [
    ...
    'AI 작업공간 pool에 등록된 Unity 프로젝트의 독립 하위 작업은 MindNProgress가 서로 다른 worker, 브랜치와 lease를 배정하므로 병렬 위임할 수 있음. 기존 대화도 새 worker CWD로 재바인딩되며 완료 결과는 체크포인트 후 main에 직렬 통합됨. 통합 충돌은 main을 건드리지 않고 해당 작업을 수행한 같은 AI 대화를 worker 통합 브랜치에서 재개해 해결·검증하며, 통합이 끝난 뒤에만 상위 대화를 재개함. pool 미등록 프로젝트이거나 WORKSPACE_POOL_UNAVAILABLE 응답을 받은 경우에는 같은 작업공간 충돌을 피하도록 순차 위임함',
...
async function main() {
  ...
  registerTool(server, 'mindnprogress_delegate_ai_work', '이 대화가 시작된 카드의 계층상 하위 카드 AI 대화에 구체적인 작업을 위임합니다. 직계 자식뿐 아니라 모든 깊이의 하위 카드를 지원하며, 다른 카드를 get_context로 조회해도 위임 기준 카드는 바뀌지 않습니다. 기존 대화를 이어가거나 새 대화를 만들 수 있습니다. 등록된 AI 작업공간 pool은 독립 worker를 자동 배정하고 완료 변경을 main에 직렬 통합합니다. 통합 충돌은 같은 하위 AI가 worker에서 해결하며, 실제 통합과 최종 검증이 끝난 뒤에만 결과를 포함한 메시지로 현재 상위 AI 대화를 자동 재개합니다. 먼저 후보 목록과 작업 상태를 확인하고, 현재 문서 version을 sourceRevision으로 전달하세요.', {
