...
3. `총괄 AI 대화 시작`에서 전체 진행 방향을 제안받습니다. 총괄 AI는 최신 기획 기준·전체 목표·공통 지침과 기존 문서를 읽기 전용으로 분석하여 범위, 문서 분할·책임, 기존 구현 활용, 순서와 위험을 제안한 뒤 사용자 승인을 기다립니다. 승인 전에는 문서·카드 정비나 AI 위임을 하지 않도록 전문에 명시합니다.
4. 전체 방향 승인 후에도 문서별 실행 계획은 별도로 승인받습니다. `총괄 AI에 위임 제안 요청`은 대상·계획 버전·작업 방향·변경 범위·허용 작업·제외 범위·완료 조건과 실제 전달할 전문의 제안 요청입니다. 버튼 클릭이나 전체 방향 승인만으로 문서 실행이 승인되지는 않습니다. 일부 문서만 승인하면 해당 문서·작업만 진행합니다.
5. 문서 담당·하위 AI는 사용자 요청 또는 상위 AI가 전달한 범위를 수행하며 같은 사용자 승인을 반복해서 요구하지 않습니다. 분석·제안만 맡겼다면 구현으로 확대하지 않습니다. 원본 전수 등록·소유권·버전 차이·기존 구현 감사·정책 Ref 준비가 끝난 구현은 하위 카드 AI에 위임합니다. 그룹→문서 위임은 분석·조정 전용으로 worker를 점유하지 않습니다.
6. 그룹 개요에서 문서별 AI 상태, 하위 업무 집계, 위임·복구 상태와 결과를 확인합니다. 하위 업무가 남아 있으면 그룹 위임은 문서 검수까지 기다린 뒤 총괄 AI에 결과를 전달합니다. 일반·문서 담당 AI는 결과를 검증하고 맡긴 범위의 남은 작업을 이어갑니다. 그룹 총괄은 결과 회수 뒤에도 두 단계 승인을 유지하며 방향·범위·기획 기준 변경이나 승인 근거가 불명확한 경우 사용자에게 재제안합니다.

그룹 총괄은 사용자가 승인한 제안·대상·허용 작업과 실제 승인 발언 및 확인 가능한 대화·메시지 출처를 확인하여 전달합니다. 총괄 AI가 사용자 승인을 대신하거나 승인 근거를 추측해서 만들면 안 됩니다. 그룹 총괄의 미승인 제안은 대화로만 보고하며 카드·댓글·공유 지식·대기 항목을 변경하지 않습니다. 제안 작성이나 위임 응답이 끝났다는 사실은 개발 완료나 총괄의 다음 실행 승인이 아닙니다.
...
그룹 설정은 버전 충돌을 검사하여 다른 편집자의 변경을 덮어쓰지 않습니다. 진행 중인 그룹 위임의 총괄·대상 문서는 그룹 이동이나 휴지통 이동이 제한됩니다. 업무 카드 완료 수와 AI 실행 완료는 기획 전체 완료를 뜻하지 않으며, 통합 관리 문서의 원본 요구사항·소유권·검증 증거로 최종 판정합니다. 전체 요구사항 자동 검사 7종과 새 기획서에 대한 정식 적용 검증은 별도 후속 범위입니다.
...
- 재개는 현재 문서·그룹 기준과 기존 승인 범위 확인이 필요합니다. 새 그룹 위임은 기획 기준 버전을 보존하며 기준이 변경되면 재개를 막습니다. 기준 버전이 없는 과거 위임은 사용자가 기존 승인 계획을 직접 확인하고, 수신 AI도 원 승인 근거를 확인해야 합니다. 승인 범위 확장이나 모델 자동 전환은 하지 않습니다.
...
| `mindnprogress_get_group_context` | 그룹의 기획 기준, 총괄 문서, 소속 문서 루트와 위임 결과를 조회합니다. |
...
그룹 총괄 루트의 `mindnprogress_delegate_ai_work`는 `mapId`에 총괄 문서, `targetMapId`와 `targetCardId`에 소속 문서와 루트, `sourceRevision`과 `targetRevision`에 두 문서의 최신 버전을 받습니다. `targetMapId`를 생략한 기존 호출은 같은 문서의 계층상 하위 카드에 위임합니다. 그룹 설정은 `server/data/_group-projects`에 별도로 저장되어 이전 클라이언트의 목록 정렬 저장에도 유지되고 전체 데이터 백업에 포함됩니다.
