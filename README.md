...
- AI 위임이 한도·복구 필요·통합 정리 대기로 막혔을 때의 `⛔ AI 위임 차단 알림`과 하위 카드의 AI 작업이 완료됐을 때의 `✅ AI 위임 완료 알림`
- 전체 활성 문서의 공유 지식 길이·구조·정확 중복·지식선 소비 현황을 원문 노출과 문서 변경 없이 조회

### 그룹 단위 기획·개발 관리

그룹 이름을 누르면 그룹 개요를 열고, 왼쪽 화살표로 문서 목록을 접거나 펼칩니다. 기존 문서를 드래그해 편입할 수 있으며 편입만으로 AI가 실행되지는 않습니다.

1. 그룹 개요에서 기획 원본 링크 또는 AI가 접근할 수 있는 파일 경로, 기준 버전, 전체 목표와 공통 지침을 기록합니다.
2. `총괄 준비`에서 통합 관리 문서를 새로 만들거나 그룹 안의 기존 문서를 연결합니다. 총괄 대화와 요구사항 소유권 원장은 이 문서에 보관합니다.
3. `총괄 AI 대화 시작`에서 전체 진행 방향을 제안받습니다. 총괄 AI는 최신 기획 기준·전체 목표·공통 지침과 기존 문서를 읽기 전용으로 분석하여 범위, 문서 분할·책임, 기존 구현 활용, 순서와 위험을 제안한 뒤 사용자 승인을 기다립니다. 승인 전에는 문서·카드 정비, 문서 지시 전달이나 AI 위임을 하지 않도록 전문에 명시합니다.
4. 전체 방향 승인 후에도 문서별 실행 계획은 별도로 승인받습니다. `지시 전문 제안`은 대상·계획 버전·작업 방향·변경 범위·허용 작업·제외 범위·완료 조건과 실제 전달할 지시 전문의 제안 요청입니다. 버튼 클릭이나 전체 방향 승인만으로 문서 실행이 승인되지는 않습니다. 일부 문서만 승인하면 해당 문서·작업만 진행합니다.
5. 총괄 AI는 승인된 문서별 계획을 `mindnprogress_send_group_document_instruction`으로 해당 문서의 루트 AI에 전달합니다. 이 전달은 AI 작업 위임이나 worker 배정이 아닙니다. 문서 담당 AI가 전달받은 범위를 수행하고, 원본 전수 등록·소유권·버전 차이·기존 구현 감사·정책 Ref 준비가 끝난 구현을 자기 문서의 하위 카드 AI에 위임합니다.
6. 그룹 개요와 MCP 문맥에서 문서별 AI 상태, 하위 업무 집계, 문서 지시의 대기·전달·응답 상태와 문서 내부 위임 결과를 확인합니다. `queued`, `delivered`, `replied`는 전달 상태일 뿐 업무 완료가 아닙니다. 그룹 총괄은 실제 카드와 검증 근거를 확인하고 방향·범위·기획 기준 변경이나 승인 근거가 불명확한 경우 사용자에게 재제안합니다.

그룹 총괄은 사용자가 승인한 제안·대상·허용 작업과 실제 승인 발언 및 확인 가능한 대화·메시지 출처를 확인하여 전달합니다. 총괄 AI가 사용자 승인을 대신하거나 승인 근거를 추측해서 만들면 안 됩니다. 그룹 총괄의 미승인 제안은 대화로만 보고하며 카드·댓글·공유 지식·대기 항목을 변경하지 않습니다. 제안 작성이나 지시 응답이 끝났다는 사실은 개발 완료나 총괄의 다음 실행 승인이 아닙니다.

그룹 승인 절차는 AI 전문의 행동 규칙이며 서버가 승인 토큰을 검사해 모든 변경·위임 API를 거부하는 강제 승인 기능은 아닙니다. 일반 `AI 대화 시작`의 기본 요청은 확정 내용의 카드 정비·저장 확인 후 다음 작업을 제안하는 이전 전문으로 복구했습니다. 일반·하위 대화의 시작, MCP 공통 안내·선택 카드 문맥, 위임·복구·결과 수신에는 그룹 총괄의 승인 대기 전문을 주입하지 않습니다. 그룹 총괄 여부는 그룹 소속 자체가 아닌 총괄 문서 루트와 대화 용도로 구분합니다. Dooray 승인 대화의 전용 서버 승인 검증, 작업공간·위임 계층·결과 원문 무결성 검증은 유지합니다.

새 전문 적용에는 서버·MCP 재시작/재연결과 화면 새로고침이 필요하며, 이미 실행 중인 대화·전달된 전문·대기열 작업·기존 카드 본문은 자동 변경하거나 중지하지 않습니다. 이미 저장한 완료 전문은 해시·재시도 일관성을 위해 그대로 재사용합니다. 기존 대화를 계속 사용할 때는 변경된 역할별 지침을 별도로 전달하거나 새 대화를 시작해야 합니다. 사용자 전역 지침과 별도 스킬 규칙은 이 전문 복구로 변경하지 않습니다.

그룹 설정은 버전 충돌을 검사하여 다른 편집자의 변경을 덮어쓰지 않습니다. 전달 대기 중인 문서 지시와 진행 중인 과거 그룹 위임의 총괄·대상 문서는 그룹 이동이나 휴지통 이동이 제한됩니다. 업무 카드 완료 수와 AI 실행 완료는 기획 전체 완료를 뜻하지 않으며, 통합 관리 문서의 원본 요구사항·소유권·검증 증거로 최종 판정합니다. 전체 요구사항 자동 검사 7종과 새 기획서에 대한 정식 적용 검증은 별도 후속 범위입니다.
...
- 로그인하지 않은 공유 방문자를 위한 공개 읽기 전용 접근(기본 비활성화, `MNP_PUBLIC_VIEWER_ENABLED=true`로 명시적으로 활성화)
- 공개 뷰어의 문서·댓글 변경 요청을 API 서버에서도 차단하고, 비활성화 시 공개 뷰어 세션과 API 접근을 모두 거부
...
- 위임한 카드의 진행률 요약 자리에 표시되는 **AI 위임 중 / AI 위임 복구 필요**와 카드 세부 정보의 **AI 위임 진행 현황 / AI 작업 복구**에서 위임 상태를 직접 확인하고 재개할 수 있습니다. 위임받은 하위 카드에는 상위 카드 소유의 위임 현황을 중복 표시하지 않습니다. AI가 실행되지 않아도 편집자가 사용할 수 있으며, 같은 상위 카드에 연결된 새 AI 대화도 복구를 요청할 수 있습니다.
- 사용량 중단은 변경 유무와 관계없이 작업공간 소유권을 보존합니다. 동일 세션·브랜치·HEAD이며 진행 중인 Git 작업이나 충돌이 없으면 커밋 전 변경도 그대로 이어갑니다. 다른 위임이나 서버 재시작 과정에서 이 작업공간을 자동 반납하지 않습니다.
- 구버전에서 `failed-clean`으로 이미 반납한 한도 중단은 풀에 남은 소유권·실패 기록을 검증한 뒤 복구용 lease를 배정합니다. 같은 대화를 사용하되 새 전문의 배정 경로를 따르며, 과거 lease와 결과는 이력에 보존합니다. 빈 작업공간이 없으면 배정을 거부하고 기존 대기를 유지합니다.
- 원인과 복구 경계, 검증 시나리오는 [AI 한도 중단 복구 검토](docs/ai-limit-recovery.md)에 정리했습니다.
- 사용량·요청 제한은 `waiting-usage-limit` / `waiting-rate-limit`로 보존하며 AI를 자동 재호출하지 않습니다. 총괄→문서 조정은 worker 없이도 같은 위임 ID·대화에서 복구할 수 있습니다.
- 그룹의 **문서 위임과 결과**에서 `상태 다시 확인`, `승인 범위 작업 재개`, `결과 전달 재시도`를 구분합니다. 상태 확인은 같은 operation만 조회하며, 다른 턴의 대화 종료나 카드 진행률을 임의로 위임 완료로 취급하지 않습니다. 직접 이어간 대화의 결과는 재개 요청에서 기존 결과부터 확인하도록 전달합니다.
- 작업은 완료됐으나 상위 AI 전달만 실패한 경우 `작업 완료 · 총괄 보고 대기`로 표시합니다. 결과 전달 재시도는 하위 작업을 재실행하지 않으며, 완료된 하위 업무를 보고 실패만으로 미완료 집계하지 않습니다. 캡처된 결과 원문이 없거나 해시·실행 턴 무결성이 맞지 않으면 같은 대화의 최신 응답으로 대체하지 않고, 원문이 제외됐음을 밝힌 작업공간·체크포인트·통합 메타데이터만 전달합니다.
- 문서 조정 실행 자체가 끝났지만 의도적으로 남긴 하위 업무·외부 대기 때문에 `waiting-document-work`에 머무르면 `대기 유지하고 조정 종료`로 카드와 실제 미완료 하위 위임을 그대로 보존한 채 현재 결과만 총괄 AI에 전달할 수 있습니다. 이때 변경 없이 한도에 막힌 과거 시도와 같은 카드의 완료된 후속 위임이 확인되면 과거 시도는 후속 위임 ID를 남긴 `superseded`로 함께 정리합니다. 재시작 뒤 과거 operation이 사라져 `recovery-required`로 후퇴한 구버전 기록도 결과 해시가 보존되어 있으면 같은 방법으로 복구합니다. MCP에서는 사용자의 명시적인 요청 후 `mindnprogress_finalize_ai_coordination`을 사용합니다.
- 사용량·요청 한도로 변경 없이 끝난 과거 시도를 새 위임이 같은 카드에서 실제 완료했다면, `mindnprogress_supersede_ai_delegation`으로 성공한 후속 위임 ID를 감사 이력에 남기고 과거 시도만 `superseded`로 종료할 수 있습니다. 과거 작업공간에 보존할 변경이 있거나 후속 위임의 작업·보고가 완료되지 않았으면 거부합니다.
- 재개는 현재 문서·그룹 기준과 기존 승인 범위 확인이 필요합니다. 새 그룹 문서 지시는 그룹·발신·대상 문서 버전을 보존하며 기준이 변경되면 전달을 막습니다. 교차 문서 AI 위임의 신규 생성은 중단하고 과거 이력의 조회·복구만 유지합니다. 기준 버전이 없는 과거 위임은 사용자가 기존 승인 계획을 직접 확인하고, 수신 AI도 원 승인 근거를 확인해야 합니다. 승인 범위 확장이나 모델 자동 전환은 하지 않습니다.
- 복구 이력에 과거 오류·결과를 보존합니다. 복구·보고 요청의 operation ID는 실행 전에 저장하여 응답 유실·MnP 재시작 뒤 중복 실행을 막습니다. 전달 여부가 불명확하면 `복구 요청 전달 확인 대기`로 표시하고 기존 요청을 조회하며 새 실행을 만들지 않습니다.
- 새로 연결한 MCP에는 `mindnprogress_refresh_ai_delegation`, `mindnprogress_retry_ai_delegation_report`, `mindnprogress_finalize_ai_coordination`, `mindnprogress_supersede_ai_delegation`이 추가됩니다. 기존 `mindnprogress_recover_ai_delegation`은 사용자 요청 후 기존 승인 범위의 실행 복구에 사용합니다.
- 서버 업데이트 후 MnP 재시작이 필요합니다. 기존 사용량 제한 실패 기록은 재시작 시 이력을 보존한 대기 상태로 전환하며 작업을 자동 실행하거나 완료로 바꾸지 않습니다. MCP 도구 목록 갱신은 MCP 재연결이 필요합니다.
...
백업에는 `server/data` 전체와 저장소 루트에 존재하는 로컬 `.env*`·`*.local` 설정이 포함됩니다. 저장소 밖 사용자 설정 폴더(`~/.mnp`)의 설정은 포함되지 않으므로 필요하면 따로 보관하세요. 문서, 계정, 세션, 댓글, 알림, 변경 이력, 일일 백업, 문서별 원본 이미지, MCP 토큰과 AI 작성자 귀속을 함께 보존하며 `node_modules`, `dist`, 로그와 PID 같은 재생성 가능한 파일은 제외합니다. 서버가 실행 중이면 일관된 시점의 데이터를 복사하기 위해 잠시 중지하고, 스냅샷 복사가 끝나는 즉시 기존 실행 상태로 자동 복구한 뒤 압축과 검증을 계속합니다. ZIP 검증이 끝나면 서버를 다시 잠시 중지하고 현재 문서·변경 이력·일일 백업 어디에서도 참조하지 않는 24시간 이상 된 이미지 자산만 정리한 뒤 실행 상태를 복구합니다. 삭제 대상도 정리 직전 ZIP에는 포함되므로 필요하면 백업에서 복원할 수 있습니다.

각 ZIP에는 파일별 크기와 SHA-256을 기록한 `manifest.json`과 수동 복원 안내 `RESTORE.txt`가 포함됩니다. 백업 배치는 .NET ZIP API의 `Fastest` 수준으로 압축하며, 기존 `Optimal` 수준보다 압축 속도를 우선하므로 ZIP 크기는 커질 수 있습니다. 검증할 때는 ZIP 내부 파일을 스트림으로 읽어 크기와 SHA-256을 manifest와 대조한 뒤에만 완료 처리합니다. 검증용 압축 해제 폴더를 만들지 않으며, ZIP 구조와 기존 복원 배치의 호환성은 유지합니다. 복원은 동일한 소스 버전을 준비한 후 다음처럼 실행합니다.
...
터미널에서 처음 실행할 때는 `chmod +x MindNProgress_Backup.command MindNProgress_Restore.command`로 실행 권한을 설정합니다. macOS가 파일을 차단하면 Finder에서 Control-클릭 후 `열기`를 선택합니다.
...
| `MNP_DOORAY_API_KEY` | 없음 | Dooray API 키 우선 지정. `DOORAY_API_KEY`와 `DOORAY_API_TOKEN`도 지원하며, 모두 없으면 Claude와 AionUi의 Dooray MCP 등록을 순서대로 조회 |
...
| `MNP_DOORAY_MCP_SERVER_NAME` | `docker-dooray-mcp` | Claude 또는 AionUi 설정에서 먼저 조회할 Dooray MCP 서버 이름. 없으면 이름에 `dooray`가 포함되고 키가 있는 등록을 탐색 |
...
PC별 설정은 저장소 밖 MindNProgress 설정 폴더 `~/.mnp`의 `mnp.conf`에 둡니다. 형식은 `.env.local`과 같은 `KEY=VALUE`이며 `#` 주석을 쓸 수 있습니다. 공개 도메인처럼 Git 저장소에 남기면 안 되는 값은 이 파일에 넣으세요. `npm run dev`, `npm run dev:server`, `npm start`와 Vite 설정이 이 파일을 읽습니다.

저장소 루트의 `mnp.conf.sample`에 쓸 수 있는 키와 설명이 들어 있습니다. 처음에는 이 파일을 복사한 뒤 이 PC에 맞게 고칩니다.

```bash
mkdir -p ~/.mnp
cp mnp.conf.sample ~/.mnp/mnp.conf
```

`~/.mnp/mnp.conf`가 없으면 `mnp.conf.sample`을 대신 읽습니다. 샘플에는 서버 기본값과 같은 값(`MNP_API_HOST`, `MNP_API_PORT`, `MNP_WEB_PORT`)만 켜져 있고 공개 주소 같은 PC별 값은 주석으로만 들어 있으므로, 복사하지 않아도 기본 동작은 달라지지 않습니다. 실제 도메인은 샘플이 아니라 `~/.mnp/mnp.conf`에 쓰세요. 서버를 시작하면 콘솔의 `설정 파일:` 줄에서 실제로 읽은 파일을 확인할 수 있습니다.

`MNP_PUBLIC_URL`의 호스트는 Vite 개발 서버의 허용 호스트에 자동으로 추가되므로, 리버스 프록시가 원본 Host를 보존해 개발 서버로 넘기는 구성에서도 도메인으로 접속할 수 있습니다.

저장소 루트의 Git 제외 파일 `.env.local`도 계속 지원합니다. 같은 키가 여러 곳에 있으면 OS나 실행 배치에서 지정한 환경변수, `.env.local`, `~/.mnp/mnp.conf`(없으면 `mnp.conf.sample`) 순으로 우선합니다. 한 PC에서 여러 클론을 쓸 때는 공통 값을 `~/.mnp/mnp.conf`에, 클론별 값을 `.env.local`에 둘 수 있습니다. 테스트가 서버 엔트리를 직접 실행할 때는 이 설정 파일들을 읽지 않으므로 개인 작업공간과 격리됩니다. 설치본의 Dev 배치는 작업공간 구성 경로를 직접 전달하므로 이 파일들이 없어도 됩니다.

카드의 `taskUrl`은 Dooray를 포함한 범용 업무 링크로 유지합니다. 일반 카드에 Dooray 업무 URL을 설정하면 카드 형태와 업무 문맥은 그대로 유지하고 제목 왼쪽에 Dooray 아이콘, 오른쪽에 원본 열기 아이콘을 표시합니다. 다른 웹 URL이면 일반 카드 표현을 그대로 유지합니다. 마인드맵 캔버스에 Dooray 업무 URL을 직접 붙여넣으면 MNP 서버가 업무 제목과 상태를 조회해 크기 조절 가능한 전용 Dooray 지식 카드로 추가합니다. 전용 카드의 Dooray 제목과 원본 정보는 저장된 값을 즉시 표시하며 제목은 편집할 수 없습니다. 원본 URL은 변경할 수 있고, 새 URL 확인에 성공하면 URL·제목·상태를 한 번에 교체합니다. 편집자가 문서를 열거나 카드 상세 보기를 열면 원본을 비동기로 다시 조회하고, 실제 변경이 있을 때만 저장 값을 갱신합니다. 조회에 실패하면 기존 저장 값을 유지합니다. 사용자는 AI가 지식으로 활용할 보충 설명을 입력하고 주요·보조 지식선으로 일반 카드에 연결합니다. 서버는 `MNP_DOORAY_API_KEY`, `DOORAY_API_KEY`, `DOORAY_API_TOKEN` 순서로 환경변수를 확인합니다. 값이 없으면 `MNP_DOORAY_CONFIG_FILE`의 Claude MCP 설정에서 지정한 서버 이름을 먼저 찾고, 이어 이름에 `dooray`가 포함된 등록을 탐색합니다. Claude 설정에도 키가 없으면 실행 중인 AionUi의 MCP 등록을 같은 규칙으로 조회합니다. Suite 설치본에서 Dooray MCP를 선택하면 Dev 런처가 사용자별 DPAPI 파일을 복호화해 `MNP_DOORAY_API_KEY`로 MnP 프로세스에 전달하므로 별도 평문 설정이 필요하지 않습니다. API 키는 브라우저, 문서 데이터와 API 응답에 포함되지 않습니다. 사용자 홈의 `.claude.json`은 저장소 밖에 있으므로 Git 커밋이나 MindNProgress 백업에 포함되지 않습니다.
...
다른 PC의 브라우저에서 `AI 대화 열기`를 누르면 AionUi WebUI의 `/#/conversation/{대화 ID}`를 매번 새 탭으로 엽니다. 닫힌 탭의 이름을 브라우저가 보존해 다음 실행이 무반응이 되는 상황을 피하기 위한 동작입니다. 기본 WebUI 주소는 `MNP_PUBLIC_URL`과 같은 호스트의 7777 포트이며, 주소나 포트가 다르면 `MNP_AIONUI_WEB_URL`을 지정합니다. 서브 머신에서 시작한 대화는 인증된 Runner 요청에서 관측한 해당 머신 주소와 같은 WebUI 포트를 조합해 엽니다. 이때 AionUi가 그 주소에서 원격 접속을 허용하도록 실행되어 있어야 합니다. `localhost` 또는 `127.0.0.1`로 접속한 브라우저는 기존 AionUi 데스크톱 딥링크를 사용하지만, `MNP_AIONUI_WEB_URL`을 명시하면 WebUI 열기가 우선됩니다.
...
MNP에서 `AI 대화 시작`으로 AionUi에 전달하는 신규 대화 전문과 하위 AI 위임 전문은 `config/ai-delegation-instruction.md`를 공통 원본으로 사용합니다. 사용자별로 바꾸려면 `~/.mnp/ai-delegation-instruction.md`를 사용하며, 지시문을 바꾼 뒤에는 MNP 서버를 재시작하고 MNP 브라우저 화면을 새로고침해야 합니다. 이미 만들어진 AionUi 대화의 최초 전문은 자동으로 바뀌지 않습니다.
...
Unity 작업공간 풀이 메인 PC 한 대에 있으면 병렬 처리량이 그 PC의 한계에 묶입니다. 맥북 같은 다른 장비를 서브 머신으로 붙여 작업을 분산할 수 있습니다.

MindNProgress는 **메인 머신 한 대에서만 실행합니다.** 서브 머신에는 문서 데이터도 웹 화면도 두지 않고, 그 장비의 AionUi와 Unity 작업공간 풀 그리고 작은 중계 프로세스인 **Runner**만 둡니다.

호출 방향은 서브 머신에서 메인 머신으로 나가는 쪽뿐입니다. 메인이 서브의 AionUi를 직접 호출하지 않고 Runner가 처리할 요청을 long-poll로 가져갑니다. 그래서 서브 머신은 inbound 포트를 열지 않고, 인증이 없는 AionUi 내부 API가 사내망에 노출되지 않으며, 서브 머신의 IP가 바뀌어도 설정을 고칠 필요가 없습니다.

서브 머신에서 새 WebUI 대화를 시작할 때 AionCore의 완료 콜백은 Runner가 임의 포트의 `127.0.0.1` 전용 릴레이로 받습니다. Runner는 이 콜백만 기존 인증된 outbound 연결로 메인 MnP에 전달합니다. LAN 인터페이스나 방화벽 포트를 열지 않으며, 하트비트·long-poll에 릴레이 주소가 없는 구형 Runner에서는 새 대화 발급을 거부해 연결 누락을 방지합니다.

서브 머신의 MindNProgress MCP도 로컬에 별도 MnP 서버를 요구하지 않습니다. AionUi Runner가 사용자 전용 임시 파일로 알리는 `127.0.0.1` 중계를 자동 발견하고, Runner의 머신 자격 증명으로 메인 MnP에 요청합니다. 전역 연동 토큰은 서브 머신에 복사하지 않습니다. 메인 PC에서는 로컬 API와 로컬 연동 토큰을 항상 먼저 사용하므로 기존 MCP 동작이 유지되며, 외부 브라우저 요청은 Runner 전용 헤더가 없으므로 이 중계 인증의 영향을 받지 않습니다.

### 사용 준비

이 기능은 계정마다 따로 켭니다. 켜지 않은 계정은 기존과 동일하게 메인 머신에서만 대화와 위임이 실행됩니다.

Runner는 브라우저가 쓰는 것과 같은 공개 주소로 접속합니다. 개발 서버가 모든 인터페이스에 바인딩되어 `/api`를 로컬 API로 프록시하므로 **API 포트를 따로 열거나 `MNP_API_HOST`를 바꿀 필요가 없습니다.** 자동 감지된 주소가 실제 접근 주소와 다르면 `MNP_PUBLIC_URL`을 지정합니다. 이 주소로 다른 장비가 접속할 수 없는 상태라면 분산 작업 설정 화면이 먼저 알려 줍니다.

1. 서브 머신에서 이 기능이 포함된 **AionUi 데스크톱 앱**을 실행합니다. AionCore는 기존 내부 API를 그대로 사용하므로 별도 재빌드가 필요하지 않습니다.
2. 서브 머신의 브라우저로 메인 머신의 MindNProgress에 접속하고 계정 메뉴 → `분산 작업 설정`을 엽니다. 메인 머신은 서버가 기동할 때 자동으로 등록되어 있습니다.
3. `서브 머신 등록`에 머신 ID와 이름을 입력합니다. 머신 ID는 소문자, 숫자, 하이픈만 사용합니다. **등록한 사람이 소유자가 되며 위임은 본인 소유 머신에만 보낼 수 있습니다.**
4. **서브 머신에서 연 브라우저에서** 등록한 머신의 `AionUi 연결`을 누릅니다. 다른 장비에서 누르면 그 장비의 AionUi가 연결되므로 대상 장비가 맞는지 확인해야 합니다.
5. macOS가 AionUi 열기를 물으면 허용합니다. AionUi의 `설정 → 연동 → MindNProgress Runner`에서 서버 주소와 머신 ID를 확인한 뒤 연결을 승인합니다.
6. MindNProgress에서 `서브 머신에 작업 분산 사용`을 켜고 기본 실행 머신을 고른 뒤 저장합니다.
7. 머신 목록이 `연결됨`으로 바뀌는지 확인합니다. `연결 확인`을 누르면 Runner를 거쳐 그 머신의 AionUi까지 실제로 닿는지 확인합니다.
8. 재부팅 뒤에도 자동으로 연결하려면 AionUi의 `설정 → 시스템 → 시작 시 자동 실행`을 켭니다. macOS 로그인 시 AionUi가 시작되고 Runner도 함께 복구됩니다.

머신 목록의 서브 머신에는 마지막 접속 시각과 함께 `연결됨` 또는 `끊김`이 표시되며 화면을 열어 둔 동안 갱신됩니다. 하트비트나 요청 대기가 일정 시간 없으면 `끊김`으로 표시됩니다.

메인 머신 등록과 삭제는 관리자만 할 수 있고, 관리자는 다른 사용자가 등록한 서브 머신도 정리할 수 있습니다.

### AionUi 사이드카 동작

Runner는 AionUi에 포함된 별도 utility process로 실행됩니다. 한 번 연결하면 터미널 명령을 붙여넣거나 재부팅할 때 Runner를 따로 실행할 필요가 없습니다.

- AionUi의 로컬 AionCore가 준비된 뒤 Runner를 자동 시작합니다.
- Runner가 예기치 않게 종료되면 자동으로 재시작합니다. 서버가 인증을 거부한 경우에는 무한 재시작하지 않고 설정 화면에 오류를 표시합니다.
- AionUi 종료 시 새 요청을 받지 않고 이미 가져온 요청의 결과를 정리한 뒤 종료합니다.
- 연결 정보가 없는 AionUi에서는 Runner 프로세스를 시작하지 않습니다.
- 브라우저 전용 AionUi WebUI에는 Runner 설정 메뉴가 표시되지 않습니다.

연결에는 10분 동안 유효하고 한 번만 쓸 수 있는 페어링 코드를 사용합니다. 영구 Runner 토큰은 브라우저 화면, 딥링크, 주소 표시줄, 로그와 프로세스 인자에 넣지 않습니다. AionUi가 페어링 코드를 교환한 뒤 운영체제의 보안 저장소를 사용해 자격 증명을 암호화합니다. 안전한 저장소를 사용할 수 없는 환경에서는 연결을 거부합니다.

AionUi의 `연결 해제`는 해당 AionUi에 저장된 자격 증명을 삭제하고 사이드카를 종료합니다. 서버에 남은 권한까지 폐기하려면 MindNProgress의 분산 작업 설정에서 `서버 권한 폐기`를 사용합니다. 재연결하면 새 토큰을 발급하면서 이전 토큰은 무효가 됩니다.

### 수동 CLI Runner 호환

기존 자동화나 진단 환경은 이전 수동 Runner를 계속 사용할 수 있습니다. 새 설치의 기본 경로는 AionUi 사이드카이며, 수동 실행에는 기존에 발급받은 Runner 토큰이 필요합니다.

```bash
MNP_RUNNER_API_URL=http://<메인 PC 주소>:4175 \
MNP_RUNNER_MACHINE_ID=macbook \
MNP_RUNNER_TOKEN=mnprn_... \
node runner/index.mjs
```

`npm run runner`로도 실행할 수 있습니다. 기본 UI에서는 평문 토큰과 실행 명령을 더 이상 표시하지 않으므로 일반 사용자는 `AionUi 연결`을 사용합니다.

| 환경변수 | 기본값 | 설명 |
| --- | --- | --- |
| `MNP_RUNNER_API_URL` | 필수 | 메인 머신의 MindNProgress 접속 주소. 브라우저로 접속하는 주소와 같습니다 |
| `MNP_RUNNER_MACHINE_ID` | 필수 | 분산 작업 설정에서 등록한 머신 ID |
| `MNP_RUNNER_TOKEN` | 필수 | 발급받은 Runner 토큰 |
| `MNP_RUNNER_AIONUI_URL` | 자동 탐색 | 지정하지 않으면 AionUi 탐색 파일과 `127.0.0.1:1986`, `127.0.0.1:5830`을 순서대로 시도 |
| `MNP_RUNNER_CONCURRENCY` | `4` | 로컬 AionUi에 동시에 보낼 요청 수 |
| `MNP_RUNNER_HEARTBEAT_MS` | `60000` | 하트비트 간격 |
| `MNP_RUNNER_RETRY_MS` | `3000` | 메인 머신 연결 실패 시 재시도 대기 |
| `MNP_RUNNER_VERBOSE` | 꺼짐 | `1`이면 처리한 요청을 모두 기록 |

토큰은 어떤 로그에도 남기지 않습니다. 토큰을 폐기하거나 머신 등록을 삭제하면 그 머신으로 향하던 요청은 즉시 실패로 확정되고 Runner 인증도 곧바로 막힙니다.

메인 머신은 상시 가동을 전제합니다. 메인이 멈추면 서브 머신은 대기 상태가 되고, Runner는 연결이 회복될 때까지 재시도합니다.

가져간 요청은 재전달하지 않습니다. AionUi 대화 생성과 dispatch는 멱등하지 않으므로 다시 보내면 대화가 중복 생성됩니다. 대신 Runner가 로컬 AionUi 호출 실패도 결과로 올려 요청자가 상한까지 기다리지 않게 합니다. long-poll 응답이 Runner에 전달되지 못한 경우에는 그 요청이 실행되지 않았으므로 대기열로 되돌려 다음 Runner가 받습니다.
...
전체 활성 문서의 공유 지식 현황은 인증된 `GET /api/shared-knowledge/audit`로 조회합니다. `mapId` 쿼리를 지정하면 한 문서만 검사하며, 응답에는 원문 대신 길이·UTF-8 바이트·SHA-256·문단과 목록 수·정확히 반복된 문장 수·15,000자 한도 사용률·지식선 소비 카드 수가 포함됩니다. 5,000자는 관심, 8,000자는 정리 권장, 12,000자는 우선 정리 기준이며 Ref 카드는 원본 카드에서만 정리하도록 직접 처리 후보에서 제외됩니다. 15,000자를 넘는 요구사항 원장처럼 독립적인 지식 영역은 한 카드의 상한을 더 늘리지 않고 영역별 카드로 분리해 필요한 카드에만 지식선으로 연결합니다.
...
정리 후보가 있을 때는 주 1회, 그리고 주요 마일스톤 완료 후나 다른 사람·AI에게 인수인계하기 전에 현황을 점검합니다. 후보는 12,000자 이상 우선 정리, 8,000자 이상 정리 권장, 5,000자 이상 또는 정확 중복이 있는 관심 단계 순으로 검토합니다. 후보가 없으면 정리를 생략하고, 어떤 경우에도 자동 삭제·자동 축약하지 않으며 전용 문맥을 확인한 카드만 명시적으로 승인합니다. `accepted-long`은 장문 전체가 현재도 재사용에 필요하다는 예외 판단일 때만 사용하며, 본문이 바뀌지 않아도 승인 30일 후 다시 후보로 분류합니다.
...
`mindnprogress_delegate_ai_work`는 현재 대화가 시작된 카드와 같은 문서의 계층상 하위 카드에만 사용합니다. 그룹 총괄→문서 루트는 별도 지시 도구를 사용하며 지시 기록은 `server/data/_group-document-instructions.json`에 저장됩니다. 과거 `coordinationOnly` 문서 간 위임은 기존 조회·복구 호환을 위해 보존합니다. 그룹 설정은 `server/data/_group-projects`에 별도로 저장되어 이전 클라이언트의 목록 정렬 저장에도 유지되고 전체 데이터 백업에 포함됩니다.
